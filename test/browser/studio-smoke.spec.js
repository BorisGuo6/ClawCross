...
async function installMockWebSocket(page) {
  await page.addInitScript(() => {
    ...
    class MockWebSocket {
      ...
      send(data) {
        window.__clawcrossSocketSends.push({
          url: this.url,
          data: String(data || ""),
        });
      }
...
async function stubStudioNetwork(page, calls, options = {}) {
  ...
  const acpxStatusPayload = options.acpxStatusPayload || {
    available: false,
    tools: [],
  };
  const acpxSessionsPayload = options.acpxSessionsPayload || {
    ok: true,
    sessions: [],
  };
  const acpxHistoryPayload = options.acpxHistoryPayload || {
    ok: true,
    history: { entries: [] },
  };
  const acpxEnsurePayload = options.acpxEnsurePayload || {
    ok: true,
    tool: "cursor",
    ...
    runs: [
      {
        ...
        recovery: {
          ...
          suggestion:
            "Resolve the pending tool approval, then deliver the inbox or rerun the blocked step.",
        },
      },
    ],
    active_run: {
  ...
  if (options.currentRuntime)
    Object.assign(currentRuntimeState, options.currentRuntime);
  if (options.subagentRuntime)
    Object.assign(subagentRuntimeState, options.subagentRuntime);
  ...
  const discoverModelsPayload = options.discoverModelsPayload || {
  ...
  await page.route(
    /https:\/\/cdnjs\.cloudflare\.com\/.*highlight.*\.css/,
    (route) =>
      route.fulfill({
  ...
  await page.route("**/proxy_check_session", (route) =>
    json(route, { valid: true, user_id: "smoke-user" })
  );
  await page.route("**/proxy_tools", (route) => json(route, []));
  await page.route("**/proxy_oasis/topics", (route) => json(route, []));
  await page.route("**/proxy_sessions", (route) =>
    json(route, { sessions: [] })
  );
  await page.route("**/proxy_sessions_status", (route) =>
    json(route, { sessions: [] })
  );
  await page.route("**/proxy_session_history", (route) =>
    json(route, { messages: [] })
  );
  await page.route("**/proxy_tunnel/status", (route) =>
    json(route, { running: false, public_domain: "" })
  );
  await page.route("**/teams", (route) =>
    json(route, { teams: ["Smoke Team"] })
  );
  await page.route("**/proxy_visual/experts*", (route) => json(route, []));
  await page.route("**/proxy_openclaw_sessions", (route) =>
    json(route, { available: true, agents: [] })
  );
  await page.route("**/proxy_acpx_status", (route) =>
    json(route, acpxStatusPayload)
  );
  await page.route("**/proxy_acpx_sessions?*", (route) =>
    json(route, acpxSessionsPayload)
  );
  await page.route("**/proxy_acpx_session_history?*", (route) =>
    json(route, acpxHistoryPayload, acpxHistoryStatus)
  );
  ...
  await page.route("**/proxy_webot_session_runtime?*", (route) => {
    ...
    currentRuntimeState.session_id =
      sessionId || currentRuntimeState.session_id || "main-session";
  ...
  await page.route("**/proxy_webot_workflow_apply", async (route) => {
    ...
    const target =
      payload.session_id === "subagent__coder__curie"
    ...
    target.mode = {
      mode: workflow.mode,
      reason: `workflow:${workflow.preset_id}`,
    };
  ...
  await page.route("**/proxy_webot_session_inbox_deliver", async (route) => {
    ...
    currentRuntimeState.inbox = currentRuntimeState.inbox.map((item) => ({
      ...item,
      status: "delivered",
    }));
    return json(route, {
      status: "success",
      delivered: currentRuntimeState.inbox.length,
    });
  });
  await page.route("**/proxy_webot_voice", async (route) => {
    ...
    currentRuntimeState.voice = {
      ...
      last_transcript:
        payload.last_transcript || currentRuntimeState.voice.last_transcript,
    };
  ...
  await page.route("**/proxy_webot_bridge_attach", async (route) => {
    ...
    const sessionId =
      payload.session_id || currentRuntimeState.session_id || "main-session";
  ...
  await page.route("**/api/setup_status", (route) => json(route, setupStatus));
  ...
  await page.route("**/api/discover_models", (route) =>
    json(route, discoverModelsPayload)
  );
...
test("studio workflow tab and settings actions stay responsive", async ({
  page,
}) => {
  const calls = {
    importOpenClaw: 0,
  ...
  await expect
    .poll(() =>
      page.evaluate(() => localStorage.getItem("oasisTownModeEnabled"))
    )
    .toBe("0");
  await expect
    .poll(() =>
      page.evaluate(() => localStorage.getItem("oasisTownWorkspaceView"))
    )
    .toBe("town");
  ...
  await expect
    .poll(async () => {
      return page
        .locator("#page-loading-overlay")
        .evaluate((el) => el.classList.contains("loading-visible"));
    })
    .toBe(false);
  ...
  await page
    .locator(
      '#hamburger-panel button[onclick*="openSettings(); closeHamburgerMenu();"]'
    )
    .click();
...
test("studio NAT tab renders mocked WebRTC diagnostics", async ({ page }) => {
  const calls = {};
  const pageErrors = [];
  ...
  await page.addInitScript(() => {
    window.alert = () => {};
    window.confirm = () => true;
    localStorage.setItem("clawcrossStudioFirstVisitV2", "1");
    ...
    class MockRTCPeerConnection {
      ...
      async setLocalDescription(offer) {
        const candidate =
          "candidate:1 1 udp 2122260223 198.51.100.7 42424 typ srflx raddr 10.0.0.5 rport 51000 generation 0";
        this.localDescription = { ...offer, sdp: `a=${candidate}\r\n` };
        setTimeout(() => {
  ...
  await page.goto("/studio?tab=nat");

  await expect(page.locator("#chat-screen")).toBeVisible();
  await expect(page.locator("#tab-nat")).toHaveClass(/active/);
  await expect(page.locator("#page-nat")).toHaveClass(/active/);
  await expect(page.locator("#nat-check-btn")).toBeVisible();
  await expect(page.locator("#anyrouter-autolog-panel")).toBeVisible();
  await expect(page.locator("#anyrouter-config-input")).toHaveValue(
    /paste-new-api-user-here/
  );

  await page.locator("#nat-check-btn").click();
  await expect(page.locator("#nat-result-title")).toContainText("Cone-like");
  await expect(page.locator("#nat-public-ip")).toContainText("198.51.100.7");
  await expect(page.locator("#nat-public-port")).toContainText("42424");
  await expect(page.locator("#nat-app-impact")).toBeVisible();
  await expect(page.locator("#nat-servers-table")).toContainText(
...
test("studio settings export button allows keyless ollama sync", async ({
  page,
}) => {
  const calls = {
    importOpenClaw: 0,
...
test("studio ACP warmup surfaces backend errors inline", async ({ page }) => {
      ...
      providers: [
        {
          ...
          auth_status: {
            ...
            message: "runtime auth not checked",
          },
          paseo_status: {
            id: "cursor",
            provider: "cursor",
  ...
  await page.goto("/studio");
  const cursorProofDot = page.locator(
    '#oc-acp-tabs [data-acp-tool="cursor"] .oc-acp-proof-dot'
  );
  await expect(cursorProofDot).toHaveClass(/oc-acp-proof-paseo/);
  await expect(cursorProofDot).toHaveAttribute(
    "title",
    /paseo: available \/ default/
  );
  ...
  await expect(page.locator("#oc-acp-session-status")).toContainText(
    /预热失败|Warm up failed/
  );
  await expect(page.locator("#oc-acp-session-status")).toContainText(
    "cursor-agent acp"
...
test("studio webot current runtime card stays synced over bridge websocket", async ({
  page,
}) => {
  const calls = {
    importOpenClaw: 0,
  ...
  await page
    .locator(
      '#hamburger-panel button[onclick*="toggleSessionSidebar(); closeHamburgerMenu();"]'
    )
    .click();

  await expect(page.locator("#webot-current-session")).toBeVisible();
  await expect(page.locator("#webot-current-session")).toContainText(
    "Current Session"
  );
  await expect(page.locator("#webot-current-session")).toContainText(
  ...
  await expect
    .poll(() => page.evaluate(() => window.__clawcrossSocketUrls.length))
    .toBe(1);
  await expect(page.locator("#webot-current-session")).toContainText(
    "attach=ATTACH-42"
  );
  ...
  const currentSessionId = await page
    .locator("#webot-current-session .webot-current-card-caption")
    .evaluate((el) => {
  ...
  await expect(page.locator("#webot-current-session")).toContainText(
    "Bridge sync received"
  );
  await expect(page.locator("#webot-current-session")).toContainText(
    "Bridge runtime synced"
  );
  await expect(page.locator("#webot-current-session")).toContainText(
    "kairos on"
  );
  ...
  await expect(page.locator("#webot-current-session")).toContainText(
    "Purring after a bridge sync"
  );

  expect(pageErrors).toEqual([]);
});

test("studio webot runtime sidebar shows runtime state and resolves approvals", async ({
  page,
}) => {
  const calls = {
    importOpenClaw: 0,
  ...
  const runtimeHeightBefore = await runtimePanel.evaluate((el) =>
  ...
  await page.mouse.move(
    dividerBox.x + dividerBox.width / 2,
    dividerBox.y + dividerBox.height / 2 + 48,
    { steps: 8 }
  );
  await page.mouse.up();
  const runtimeHeightAfter = await runtimePanel.evaluate((el) =>
    Math.round(el.getBoundingClientRect().height)
  );
  ...
  await expect
    .poll(() =>
      page.evaluate(() =>
        localStorage.getItem("clawcrossSessionRuntimePanelHeightV1")
      )
    )
    .not.toBeNull();

  await expect(page.locator("#webot-subagent-list")).toContainText("Curie");
  await expect(page.locator("#webot-subagent-detail")).toContainText(
    "Review gate"
  );
  await expect(page.locator("#webot-subagent-detail")).toContainText(
    "/tmp/clawcross/worktree/curie"
  );
  await expect(page.locator("#webot-subagent-detail")).toContainText(
    "Flask proxy chain"
  );
  ...
  await expect(page.locator("#webot-policy-status")).toContainText(
    /Approval 已处理|Approval resolved/
  );
  await expect(page.locator("#webot-subagent-detail")).toContainText(
    /approved|APPROVED/
  );
...
test("studio webot runtime surfaces recovery hints and applies workflow presets", async ({
  page,
}) => {
  const calls = {
    importOpenClaw: 0,
  ...
  expect(calls.workflowApply[0]).toMatchObject({
    preset_id: "execution_swarm",
  });
...
test("studio builtin preset modal installs team presets", async ({ page }) => {
  ...
  await expect(page.locator("#builtin-preset-list")).toContainText(
    "明朝内阁制"
  );
  await expect(page.locator("#builtin-preset-list")).toContainText(
    "翰林院小说创作局"
  ...
  await expect
    .poll(() => page.evaluate(() => window.__openedBuiltinTeam))
    .toBe("Smoke Preset Team");
  expect(pageErrors).toEqual([]);
});

test("studio oasis swarm uses pretext-backed multiline labels", async ({
  page,
}) => {
  const calls = {
    importOpenClaw: 0,
  ...
  const metrics = await page.evaluate(() => {
    const node = buildOasisSwarmNodeMetrics({
      id: "objective-1",
      label:
        "A multilingual objective label 春天到了 بدأت الرحلة with extra context for wrapping",
      type: "objective",
      degree: 3,
          ...
          edges: [
            {
              ...
              source: "objective-1",
              target: "agent-1",
              label: "Long relationship label for pretext truncation check",
              weight: 0.8,
            },
          ],
        },
    ...
    return {
      ready: Boolean(
        ...
        typeof window.ClawcrossTextLayout.measureDisplay === "function"
      ),
      labelLineCount: node.labelLineCount,
      labelLines: node.labelLines,
      edgeLabels: Array.from(
        document.querySelectorAll("#oasis-swarm-canvas text")
...
test("oasis town runtime mounts, draws canvas, and accepts live updates", async ({
  page,
}) => {
  const calls = {
    importOpenClaw: 0,
  ...
  await page.goto("/studio");
  await expect
    .poll(() => page.evaluate(() => Boolean(window.OasisTown)))
    .toBe(true);
  ...
  const mounted = await page.evaluate(() => {
    ...
    return {
      hasRuntime: Boolean(window.OasisTown),
  ...
  await expect(
    page.locator("#oasis-town-runtime-smoke-host canvas")
  ).toBeVisible();
  await expect
    .poll(() =>
      page
        .locator("#oasis-town-runtime-smoke-host canvas")
        .evaluate((canvas) => ({
          width: canvas.width,
  ...
  const canvasState = await page.evaluate(async () => {
    const canvas = document.querySelector(
      "#oasis-town-runtime-smoke-host canvas"
    );
